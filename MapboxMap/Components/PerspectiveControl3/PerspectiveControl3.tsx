...
export const PerspectiveControl3 = ({
  ...
  function addControl() {
    ...
    const ctrl = new CustomControl(
      setTerrain,
      map,
      <FSButton onClick={handlePropChange} map={map} />,
      className
    )
...
const FSButton = ({
  ...
  const [active, setActive] = useState(!!terrainActive)

  useEffect(() => {
    onClick && onClick('perspective', !!active)
  }, [active])
